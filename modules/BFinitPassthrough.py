...
import modules.bootloader as bootloader
import modules.serial_finder as serials_find
import modules.SerialHelper as SerialHelper
...
def bf_passthrough_init(
    port,
    requestedBaudrate,
    half_duplex=False,
):
    sys.stdout.flush()
    print("======== PASSTHROUGH INIT ========")
    print(
        "  Trying to initialize %s @ %s"
        % (
            port,
            requestedBaudrate,
        )
    ...
    if not _validate_serialrx(
        ...
        serial_check.append(
            "Serial Receiver UART is inverted! Hint: set serialrx_inverted = OFF"
        )
    if not _validate_serialrx(
        rl,
        "serialrx_halfduplex",
        ...
        serial_check.append(
            "Serial Receiver UART is not in full duplex! Hint: set serialrx_halfduplex = OFF"
        )
    if (
        _validate_serialrx(
            rl,
    ...
    while True:
        ...
        if line.startswith("serial"):
            ...
            if config and (int(config.group(2)) & 64 == 64):
                print("    ** Serial RX config detected: '%s'" % line)
                SerialRXindex = config.group(1)
                if not SCRIPT_DEBUG:
                    break

    if not SerialRXindex:
        raise PassthroughFailed(
            "!!! RX Serial not found !!!!\n  Check configuration and try again..."
        )

    cmd = "serialpassthrough %s %s" % (
        SerialRXindex,
        requestedBaudrate,
    )
...
def reset_to_bootloader(
    ...
    if half_duplex:
        ...
        print("  * Using half duplex (GHST)")
    else:
        BootloaderInitSeq = bootloader.get_init_seq(
            "CRSF",
            chip_type,
        )
        print("  * Using full duplex (CRSF)")
    ...
    if target is not None:
        ...
        if rx_target == "":
            print("Cannot detect RX target, blindly flashing!")
        elif ignore_incorrect_target:
            print(f"Force flashing {flash_target}, detected {rx_target}")
            ...
            print("Verified RX target '%s'" % (flash_target))
    time.sleep(0.8)
    s.close()
...
def main(
    ...
    if args.reset_to_bl:
        returncode = reset_to_bootloader(
            args.port,
            args.baud,
            args.target,
            args.action,
