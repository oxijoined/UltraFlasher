...
def _cli_parse(
    ...
    parser = ArgumentParser(
        prog=args[0],
        usage="%(prog)s [options] package.module:app",
    )
...
class Router(object):
    ...
    def _itertokens(
        ...
        for match in self.rule_syntax.finditer(rule):
            ...
            (
                ...
                g[4:7] if g[2] is None else g[1:4]
            )
    ...
    def add(
        ...
        if is_static and not self.strict_order:
            self.static.setdefault(
                method,
                {},
            )
            self.static[method][self.build(rule)] = (
                target,
                None,
            )
            return
    ...
    def match(
        ...
        for method in methods:
            if method in self.static and path in self.static[method]:
                ...
                return (
                    target,
                    getargs(path) if getargs else {},
                )
            elif method in self.dyna_regexes:
                for (
                    combined,
                    rules,
                ) in self.dyna_regexes[method]:
                    match = combined(path)
                    if match:
                        (
                            target,
                            getargs,
                        ) = rules[match.lastindex - 1]
                        return (
                            target,
                            getargs(path) if getargs else {},
                        )
...
class Bottle(object):
    ...
    def install(
        ...
        if hasattr(
            ...
            plugin.setup(self)
        if not callable(plugin) and not hasattr(
            plugin,
            "apply",
        ):
...
class BaseRequest(object):
    ...
    def params(
        ...
        for (
            key,
            value,
        ) in self.query.allitems():
            params[key] = value
        for (
            key,
            value,
        ) in self.forms.allitems():
            params[key] = value
        return params
    ...
    def POST(
        ...
        if not self.content_type.startswith("multipart/"):
            body = tonat(
                self._get_body_string(self.MEMFILE_MAX),
                "latin1",
            )
            for (
                key,
    ...
    def is_xhr(
        ...
        """
        requested_with = self.environ.get(
...
class _ImportRedirect(object):
    def __init__(
        ...
        self.module = sys.modules.setdefault(
            name,
            imp.new_module(name),
        )
    ...
    if py3k:
        ...
        def iterallitems(
            ...
            return (
                ...
                for k, vl in self.dict.iteritems()
                for v in vl
            )
    ...
    try:
        ...
        if method.lower() == "basic":
            (
                user,
                pwd,
            ) = touni(base64.b64decode(tob(data))).split(
                ":",
                1,
            )
            return (
...
class WSGIRefServer(ServerAdapter):
    def run(
        ...
        handler_cls = self.options.get(
            "handler_class",
            FixedHandler,
        )
        server_cls = self.options.get(
            "server_class",
            WSGIServer,
        )
...
class CherootServer(ServerAdapter):
    def run(
        ...
        self.options["wsgi_app"] = handler
        certfile = self.options.pop(
            "certfile",
            None,
        )
        keyfile = self.options.pop(
            "keyfile",
            None,
        )
        chainfile = self.options.pop(
            "chainfile",
            None,
        )
...
class SimpleTemplate(BaseTemplate):
    def prepare(
        ...
        **ka,
        ...
        self.cache = {}
        enc = self.encoding
        self._str = lambda x: touni(
            x,
            enc,
        )
        self._escape = lambda x: escape_func(
            touni(
                x,
                enc,
            )
    ...
    def _include(
        ...
        **kwargs,
        ...
        return self.cache[_name].execute(
            env["_stdout"],
            env,
        )
...
def template(
    ...
    **kwargs,
    ...
    if not TEMPLATES[tplid]:
        abort(
            500,
            "Template (%s) not found" % tpl,
        )
    return TEMPLATES[tplid].render(kwargs)


mako_template = functools.partial(
    template,
    template_adapter=MakoTemplate,
)
cheetah_template = functools.partial(
    template,
    template_adapter=CheetahTemplate,
)
jinja2_template = functools.partial(
    template,
...
mako_view = functools.partial(
    view,
    template_adapter=MakoTemplate,
)
cheetah_view = functools.partial(
    view,
    template_adapter=CheetahTemplate,
)
...
ext = _ImportRedirect(
    "bottle.ext" if __name__ == "__main__" else __name__ + ".ext",
    "bottle_%s",
).module
...
def _main(
    ...
    for cfile in args.conf or []:
        try:
            ...
            _cli_error(
                "Unable to parse config file %r: %s"
                % (
                    cfile,
                    error,
                )
