...
class ESPLoader(object):
        ...
        try:
            for (
                ...
                last_error = self._connect_attempt(
                    mode=mode,
                    usb_jtag_serial=usb_jtag_serial,
    ...
    def read_reg(
        ...
        if (
            ...
            raise FatalError.WithResult(
                "Failed to read register address %08x" % addr,
                data,
            )
        return val
    ...
    def flash_begin(
        ...
        if self.SUPPORTS_ENCRYPTED_FLASH and not self.IS_STUB:
            params += struct.pack(
                "<I",
                1 if begin_rom_encrypted else 0,
            )
        self.check_command(
    ...
    def get_security_info(
        ...
        res = struct.unpack(
            "<IBBBBBBBB" if esp32s2 else "<IBBBBBBBBII",
            res,
        )
        return {
            "flags": res[0],
    ...
    def parse_flash_size_arg(
        ...
        try:
            ...
            raise FatalError(
                ...
                % (
                    arg,
                    ", ".join(cls.FLASH_SIZES.keys()),
                )
            )

    @classmethod
    def parse_flash_freq_arg(
        cls,
        arg,
    ):
        try:
