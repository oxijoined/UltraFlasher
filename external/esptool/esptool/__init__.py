...
from .util import FatalError, NotImplementedInROMError, flash_size_bytes
...
def main(
    ...
    subparsers = parser.add_subparsers(
        dest="operation",
        help="Run esptool.py {command} -h for additional help",
    )

    def add_spi_connection_arg(
        parent,
    ):
        parent.add_argument(
            "--spi-connection",
    ...
    parser_erase_region.add_argument(
        "address",
        help="Start address (must be multiple of 4096)",
        type=arg_auto_int,
    )
    parser_erase_region.add_argument(
        "size",
    ...
    if (
        ...
        if (
            args.before != "no_reset_no_sync" and not args.passthrough
        ):  # ELRS added passthrough
            initial_baud = min(
                ESPLoader.ESP_ROM_BAUD,
                args.baud,
        ...
        def flash_xmc_startup():
            ...
            if fast_check and is_xmc_chip_strict():
                # Successful XMC flash chip boot-up detected by RDID, skipping.
                return

            sfdp_mfid_addr = 0x10
            mf_id = esp.read_spiflash_sfdp(
                sfdp_mfid_addr,
                8,
            )
            # Non-XMC chip detected by SFDP Read, skipping.
        ...
        if hasattr(
            ...
            if args.flash_size != "keep":  # TODO: should set this even with 'keep'
                ...
                if esp.IS_STUB and args.flash_size in (
                    "32MB",
                    "64MB",
                    "128MB",
                ):
...
class AddrFilenamePairAction(argparse.Action):
    ...
    def __init__(self, option_strings, dest, nargs="+", **kwargs):
        super(
            ...
            self,
        ).__init__(option_strings, dest, nargs, **kwargs)
    ...
    def __call__(
        ...
        for i in range(
            ...
            try:
                ...
                raise argparse.ArgumentError(
                    self,
                    'Address "%s" must be a number' % values[i],
                )
            try:
                argfile = open(
                    values[i + 1],
