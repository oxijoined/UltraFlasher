...
class PyStlink:
    ...
    def __init__(
        ...
        self._index = 0
        self._hard = False
        self._connector = stlinkusb.StlinkUsbConnector(
            dbg=self._dbg,
            serial=self._serial,
            index=self._index,
        )
    ...
    def initialize_comms(
        ...
        if self.stlink.coreid == 0:
            raise stlinkex.StlinkException(
                "STLink could not connect to microcontroller"
            )
        self._core = stm32.Stm32(
            self.stlink,
            dbg=self._dbg,
        )
    ...
    def initialize_stlink_comms(
        self,
    ):
        self.stlink = stlinkv2.Stlink(
            self._connector,
            dbg=self._dbg,
        )
    ...
    def find_mcus_by_flash_size(
        self,
    ):
        self._flash_size = self.stlink.get_debugreg16(
            self._mcus_by_devid["flash_size_reg"]
        )
        self._mcus = []
        ...
        if not self._mcus:
            raise stlinkex.StlinkException(
                "Connected CPU with DEV_ID: 0x%03x and FLASH size: %dKB is not supported. Check Protection"
                % (
                    self._mcus_by_devid["dev_id"],
                    self._flash_size,
                )
            )
    ...
    def filter_detected_cpu(
        ...
        if not cpus:
            raise stlinkex.StlinkException(
                "Connected CPU is not %s but detected is %s %s"
                % (
                    ",".join(expected_cpus),
    ...
    def load_driver(
        ...
        if flash_driver == "STM32FP":
            self.driver = stm32fp.Stm32FP(
                self.stlink,
                dbg=self._dbg,
            )
        elif flash_driver == "STM32FPXL":
            self.driver = stm32fp.Stm32FPXL(
                self.stlink,
                dbg=self._dbg,
            )
        elif flash_driver == "STM32FS":
            self.driver = stm32fs.Stm32FS(
                self.stlink,
                dbg=self._dbg,
            )
        elif flash_driver == "STM32L0":
            self.driver = stm32l0.Stm32L0(
                self.stlink,
                dbg=self._dbg,
            )
        elif flash_driver == "STM32L4":
            self.driver = stm32l4.Stm32L4(
                self.stlink,
                dbg=self._dbg,
            )
        elif flash_driver == "STM32H7":
            self.driver = stm32h7.Stm32H7(
                self.stlink,
                dbg=self._dbg,
            )
        else:
    ...
    def read_words(
        ...
        for i in range(num_words):
            words[i] = (
                f"{data[3+(i*4)]:02x}{data[2+(i*4)]:02x}{data[1+(i*4)]:02x}{data[0+(i*4)]:02x}"
            )
        return words
    ...
    def write_word(
        ...
        if initialize_comms:
            self.initialize_comms()
        print(
            "Warning: write_word() isn't as simple to use as the -w32 function from ST-LINK_CLI.exe"
        )
    ...
    def program_flash(
        ...
        for (
            ...
            if addr is None:
                addr = start_addr
            self._mcus_by_devid["erase_sizes"]
            self.driver.flash_write(
                addr,
                data,
                erase=erase,
