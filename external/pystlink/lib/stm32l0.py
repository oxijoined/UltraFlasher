...
class Flash:
    ...
    def prg_unlock(
        ...
        # unlock keys
        self._stlink.set_debugreg32(
            self._nvm + Flash.PRGKEYR_OFFSET,
    ...
    def erase_pages(
        ...
        self._dbg.verbose(
            "erase_pages from addr 0x%08x for %d byte"
            % (
                addr,
                size,
            )
        ...
        last_addr = (addr + size + self._page_size - 1) & ~(self._page_size - 1)
