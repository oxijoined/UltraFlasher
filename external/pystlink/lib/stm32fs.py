...
from pystlink.lib import stlinkex, stm32
...
class Flash:
    ...
    FLASH_SR_BSY = 1 << 16
    FLASH_SR_ERROR_MASK = (
        FLASH_SR_WRPERR | FLASH_SR_PGAERR | FLASH_SR_PGPERR | FLASH_SR_ERSERR
    )
    ...
    def get_voltage_dependend_params(
        self,
    ):
        ...
        raise stlinkex.StlinkException(
            "Supply voltage is %.2fV, but minimum for FLASH program or erase is 1.8V"
            % self._stlink.target_voltage
        )
    ...
    def erase_all(
        ...
        self._stlink.set_debugreg32(
            Flash.FLASH_CR_REG,
            Flash.FLASH_CR_MER_BIT,
        )
        self._stlink.set_debugreg32(
            Flash.FLASH_CR_REG,
            Flash.FLASH_CR_MER_BIT | Flash.FLASH_CR_STRT_BIT,
        )
        self.wait_busy(
            self._params["max_mass_erase_time"],
            "Erasing FLASH",
        )
    ...
    def erase_sector(
        ...
        flash_cr_value |= self._params["FLASH_CR_PSIZE"] | (
            sector << Flash.FLASH_CR_SNB_BITINDEX
        )
        self._stlink.set_debugreg32(
            Flash.FLASH_CR_REG,
            flash_cr_value,
        ...
        self._stlink.set_debugreg32(
            Flash.FLASH_CR_REG,
            flash_cr_value | Flash.FLASH_CR_STRT_BIT,
        )
    ...
    def erase_sectors(
        ...
        self._dbg.bargraph_start(
            "Erasing FLASH",
            value_min=flash_start,
            value_max=flash_start + size,
        )
        sector = 0
        while True:
    ...
    def wait_for_breakpoint(
        ...
        while (
            time.time() < end_time
            and not self._stlink.get_debugreg32(stm32.Stm32.DHCSR_REG)
            & stm32.Stm32.DHCSR_STATUS_HALT_BIT
        ):
            time.sleep(wait_time / 20)
        self.end_of_operation(self._stlink.get_debugreg32(Flash.FLASH_SR_REG))

    def end_of_operation(
        self,
        status,
    ):
        if status:
...
class Stm32FS(stm32.Stm32):
    ...
    def flash_write(
        ...
        self._dbg.debug(
            "Stm32FS.flash_write(%s, [data:%dBytes], erase=%s, erase_sizes=%s)"
            % (
        ...
        if status & Flash.FLASH_SR_ERROR_MASK:
            ...
            self._stlink.set_debugreg32(
                Flash.FLASH_SR_REG,
                Flash.FLASH_SR_ERROR_MASK,
            )
            status = self._stlink.get_debugreg32(Flash.FLASH_SR_REG)
            if status & Flash.FLASH_SR_ERROR_MASK:
                raise stlinkex.StlinkException("FLASH state error : %08x\n" % status)
        params = Flash.get_voltage_dependend_params(self)
        print("Align %d" % params["align"])
        self._stlink.set_debugreg32(
            Flash.FLASH_CR_REG,
            Flash.FLASH_CR_PG_BIT | params["FLASH_CR_PSIZE"],
        )
        self._dbg.bargraph_start(
            "Writing FLASH",
            value_min=addr,
