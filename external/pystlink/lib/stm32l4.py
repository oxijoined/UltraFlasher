...
class Flash:
    ...
    FLASH_SR_ERROR_MASK = (
        FLASH_SR_PROGERR_BIT
        | FLASH_SR_WPRERR_BIT
        | FLASH_SR_PGAERR_BIT
        | FLASH_SR_SIZERR_BIT
        | FLASH_SR_PGSERR_BIT
        | FLASH_SR_FASTERR_BIT
    ...
    def erase_pages(
        ...
        self._dbg.verbose(
            "erase_pages from addr %08x for %d byte"
            % (
                addr,
                size,
            )
        ...
        last_page = (
            addr - stm32.Stm32.FLASH_START + size + self._page_size - 1
        ) // self._page_size
        self._dbg.verbose(
            "erase_pages %d to %d"
            % (
...
class Stm32L4(stm32.Stm32):
    ...
    def flash_write(
        ...
        if not cr & Flash.FLASH_CR_PG_BIT:
            raise stlinkex.StlinkException(
                "Flash_Cr not ready for programming: %08x\n" % cr
            )
        while data:
            block = data[:256]
            data = data[256:]
            self._dbg.debug(
                "Stm32l4.flash_write len %s addr %x"
                % (
                    len(block),
                    addr,
                )
