...
class Flash:
    ...
    FLASH_SR_ERROR_READ = (
        FLASH_SR_RDPERR | FLASH_SR_RDSERR | FLASH_SR_SNECCERR | FLASH_SR_DBERRERR
    )
    FLASH_SR_ERROR_MASK = (
        FLASH_SR_WRPERR
        | FLASH_SR_PGSERR
    ...
    FLASH_CR_REGS = [
        FPEC1_BASE + FLASH_CR_OFFSET,
        FPEC2_BASE + FLASH_CR_OFFSET,
    ]
    FLASH_SR_REGS = [
        FPEC1_BASE + FLASH_SR_OFFSET,
        FPEC2_BASE + FLASH_SR_OFFSET,
    ]
    FLASH_CCR1_REGS = [
        FPEC1_BASE + FLASH_CCR1_OFFSET,
        FPEC2_BASE + FLASH_CCR1_OFFSET,
    ]
    FLASH_KEYR_REGS = [
        FPEC1_BASE + FLASH_KEYR_OFFSET,
    ...
    def erase_all(
        ...
        self._stlink.set_debugreg32(
            Flash.FLASH_CR_REGS[0],
            cr,
        )
        self._stlink.set_debugreg32(
            Flash.FLASH_CR_REGS[1],
            cr,
        )
        self._stlink.set_debugreg32(
            Flash.FLASH_OPTCR,
            Flash.FLASH_OPTCR_MER,
        )
        self.wait_busy(
            20,
            bargraph_msg="Erasing FLASH",
            bank=2,
            check_qw=True,
        )
...
class Stm32H7(stm32.Stm32):
    ...
    def flash_write(
        ...
        if erase:
            full_chip = (addr == self.FLASH_START) and (len(data) == flash._flash_size)
