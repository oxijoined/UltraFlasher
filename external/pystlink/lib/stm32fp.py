...
class Flash:
    ...
    def __init__(
        ...
        if self._stlink.target_voltage < 2.0:
            raise stlinkex.StlinkException(
                "Supply voltage is %.2fV, but minimum for FLASH program or erase is 2.0V"
    ...
    def erase_page(
        ...
        self._stlink.set_debugreg32(
            Flash.FLASH_CR_REG,
            Flash.FLASH_CR_PER_BIT | Flash.FLASH_CR_STRT_BIT,
        )
        self.wait_busy(0.2)
...
class Stm32FP(stm32.Stm32):
    ...
    def flash_write(
        ...
        self._dbg.debug(
            "Stm32FP.flash_write(%s, [data:%dBytes], erase=%s, erase_sizes=%s)"
            % (
...
class Stm32FPXL(Stm32FP):
    ...
    def flash_write(
        ...
        self._dbg.debug(
            "Stm32F1.flash_write(%s, [data:%dBytes], erase=%s, erase_sizes=%s)"
            % (
                ("0x%08x" % addr) if addr is not None else "None",
                len(data),
                erase,
