...
class Stlink:
    ...
    def clean_exit(
        ...
        if self._connector.xfer_counter & 1:
            self._connector.xfer(
                [Stlink.STLINK_GET_CURRENT_MODE],
                rx_len=2,
            )
    ...
    def read_version(
        ...
        rx = self._connector.xfer(
            [
                Stlink.STLINK_GET_VERSION,
                0x80,
            ],
            rx_len=6,
            retry=2,
        ...
        if dev_ver[0:2] == "V3":
            rx_v3 = self._connector.xfer(
                [
                    Stlink.STLINK_APIV3_GET_VERSION_EX,
                    0x80,
                ],
                rx_len=16,
            )
        ...
        if dev_ver == "V2-1":
            self._ver_str += "M%d" % self._ver_mass
        if self.ver_api == 1:
            raise self._dbg.warning(
                "ST-Link/%s is not supported, please upgrade firmware." % self._ver_str
            )
        if self.ver_jtag < 32 and self._ver_api == 2:
            self._dbg.warning(
                "ST-Link/%s is not recent firmware, please upgrade first - functionality is not guaranteed."
                % self._ver_str
            )
    ...
    def read_target_voltage(
        ...
        rx = self._connector.xfer(
            [Stlink.STLINK_GET_TARGET_VOLTAGE],
            rx_len=8,
        )
        a0 = int.from_bytes(
            rx[:4],
    ...
    def leave_state(
        ...
        if rx[0] == Stlink.STLINK_MODE_DFU:
            self._connector.xfer(
                [
                    Stlink.STLINK_DFU_COMMAND,
                    Stlink.STLINK_DFU_EXIT,
                ]
            )
        if rx[0] == Stlink.STLINK_MODE_DEBUG:
            self._connector.xfer(
                [
                    Stlink.STLINK_DEBUG_COMMAND,
                    Stlink.STLINK_DEBUG_EXIT,
                ]
            )
        if rx[0] == Stlink.STLINK_MODE_SWIM:
            self._connector.xfer(
                [
                    Stlink.STLINK_SWIM_COMMAND,
                    Stlink.STLINK_SWIM_EXIT,
                ]
            )
    ...
    def set_swd_freq(
        ...
        for (
            f,
            d,
        ) in Stlink.STLINK_DEBUG_APIV2_SWD_SET_FREQ_MAP.items():
            if freq >= f:
                rx = self._connector.xfer(
                    [
                        Stlink.STLINK_DEBUG_COMMAND,
    ...
    def set_swd_freq_v3(
        ...
        rx = self._connector.xfer(
            [
                Stlink.STLINK_DEBUG_COMMAND,
                Stlink.STLINK_DEBUG_APIV3_GET_COM_FREQ,
                0,
            ],
            rx_len=52,
        )
        ...
        while i < rx[8]:
            ...
            i = i + 1
        self._dbg.verbose(
            "Using %d khz for %d kHz requested"
            % (
                freq_khz,
                freq / 1000,
            )
        ...
        cmd = [
            Stlink.STLINK_DEBUG_COMMAND,
            Stlink.STLINK_DEBUG_APIV3_SET_COM_FREQ,
        ]
        cmd.extend([0x0] * 2)
        cmd.extend(
            list(
    ...
    def debug_resetsys(
        ...
        self._connector.xfer(
            [
                Stlink.STLINK_DEBUG_COMMAND,
                Stlink.STLINK_DEBUG_APIV2_RESETSYS,
            ],
            rx_len=2,
        )
    ...
    def set_debugreg32(
        ...
        if addr % 4:
            raise stlinkex.StlinkException(
                "get_mem address %08x is not in multiples of 4" % addr
            )
        cmd = [
            Stlink.STLINK_DEBUG_COMMAND,
            Stlink.STLINK_DEBUG_APIV2_WRITEDEBUGREG,
        ]
        cmd.extend(
            list(
                addr.to_bytes(
    ...
    def get_debugreg32(
        ...
        if addr % 4:
            raise stlinkex.StlinkException(
                "get_mem address %08xis not in multiples of 4" % addr
            )
        cmd = [
            Stlink.STLINK_DEBUG_COMMAND,
    ...
    def get_reg(
        ...
        cmd = [
            Stlink.STLINK_DEBUG_COMMAND,
            Stlink.STLINK_DEBUG_APIV2_READREG,
            reg,
        ]
        rx = self._connector.xfer(
            cmd,
    ...
    def set_reg(
        ...
        cmd = [
            Stlink.STLINK_DEBUG_COMMAND,
            Stlink.STLINK_DEBUG_APIV2_WRITEREG,
            reg,
        ]
        cmd.extend(
            list(
    ...
    def get_mem32(
        ...
        if addr % 4:
            raise stlinkex.StlinkException(
                "get_mem32: Address must be in multiples of 4"
            )
        if size % 4:
            raise stlinkex.StlinkException("get_mem32: Size must be in multiples of 4")
        if size > Stlink.STLINK_MAXIMUM_TRANSFER_SIZE:
            raise stlinkex.StlinkException(
                "get_mem32: Size for reading is %d but maximum can be %d"
                % (
                    size,
                    Stlink.STLINK_MAXIMUM_TRANSFER_SIZE,
                )
            )
        cmd = [
            Stlink.STLINK_DEBUG_COMMAND,
    ...
    def get_mem8(
        ...
        if size > 64:
            raise stlinkex.StlinkException(
                "get_mem8: Size for reading is %d but maximum can be 64" % size
            )
        cmd = [
            Stlink.STLINK_DEBUG_COMMAND,
            Stlink.STLINK_DEBUG_READMEM_8BIT,
        ]
        cmd.extend(
            list(
                addr.to_bytes(
    ...
    def set_mem8(
        ...
        while datablock:
            ...
            cmd = [
                Stlink.STLINK_DEBUG_COMMAND,
                Stlink.STLINK_DEBUG_WRITEMEM_8BIT,
            ]
            cmd.extend(
                list(
                    addr.to_bytes(
    ...
    def get_mem16(
        ...
        if addr % 2:
            raise stlinkex.StlinkException(
                "get_mem16: Address must be in multiples of 2"
            )
        if len(data) % 2:
            raise stlinkex.StlinkException("get_mem16: Size must be in multiples of 2")
        if len(data) > Stlink.STLINK_MAXIMUM_TRANSFER_SIZE:
            raise stlinkex.StlinkException(
                "get_mem16: Size for writing is %d but maximum can be %d"
                % (
                    len(data),
                    Stlink.STLINK_MAXIMUM_TRANSFER_SIZE,
                )
    ...
    def set_mem16(
        ...
        if addr % 2:
            raise stlinkex.StlinkException(
                "set_mem16: Address must be in multiples of 2"
            )
        if len(data) % 2:
            raise stlinkex.StlinkException("set_mem16: Size must be in multiples of 2")
        if len(data) > Stlink.STLINK_MAXIMUM_TRANSFER_SIZE:
            raise stlinkex.StlinkException(
                "set_mem16: Size for writing is %d but maximum can be %d"
                % (
                    len(data),
                    Stlink.STLINK_MAXIMUM_TRANSFER_SIZE,
                )
        ...
        cmd = [
            Stlink.STLINK_DEBUG_COMMAND,
            Stlink.STLINK_DEBUG_APIV2_WRITEMEM_16BIT,
        ]
        cmd.extend(
            list(
                addr.to_bytes(
    ...
    def set_nrst(
        ...
        self._connector.xfer(
            [
                Stlink.STLINK_DEBUG_COMMAND,
                Stlink.STLINK_DEBUG_APIV2_DRIVE_NRST,
                action,
            ],
            rx_len=2,
        )
