...
class StlinkUsbConnector:
    ...
    def __init__(
        ...
        for dev in devices:
            for dev_type in StlinkUsbConnector.DEV_TYPES:
                if (
                    dev.idVendor == dev_type["idVendor"]
                    and dev.idProduct == dev_type["idProduct"]
                ):
                    if not serial and index == 0:
                        if self._dev:
                            multiple_devices = True
                            self._dbg.info(
                                "%2d: STLINK %4s, serial %s"
                                % (
                                    num_stlink,
                                    self._dev_type["version"],
                                    self._get_serial(),
                                )
                            )
                    self._dev = dev
            ...
            if self._dev and serial and serial == self._get_serial():
                break
            if self._dev and serial is None and index == num_stlink:
                break
        if multiple_devices:
            self._dbg.info(
                "%2d: STLINK %4s, serial %s"
        ...
        if self._dev:
            self._dbg.verbose(
                "Connected to ST-Link/%4s, serial %s"
                % (
                    self._dev_type["version"],
                    self._get_serial(),
                )
    ...
    def _write(
        ...
        count = self._dev.write(
            self._dev_type["outPipe"],
            data,
            tout,
        )
        if count != len(data):
            raise stlinkex.StlinkException(
                "Error, only %d Bytes was transmitted to ST-Link instead of expected %d"
                % (
                    count,
                    len(data),
                )
        ...
        while True:
            try:
                if len(cmd) > self.STLINK_CMD_SIZE_V2:
                    raise stlinkex.StlinkException(
                        "Error too many Bytes in command: %d, maximum is %d"
                        % (
                            len(cmd),
                            self.STLINK_CMD_SIZE_V2,
                        )
                    )
                # pad to 16 bytes
    ...
    def unmount_discovery(
        ...
        for line in out.splitlines():
            ...
            if param[0].strip() == "Mounted" and param[1].strip() == "Yes":
                is_mounted = True
            if param[0].strip() == "Device / Media Name" and param[
                1
            ].strip().startswith("MBED"):
                is_mbed = True
