...
def unquote_html_attributes(
    ...
    for tag in iter(any_tag.findall(html)):
        ...
        tag = space2.sub(
            '" >',
            tag,
        )  # preserve 1 white space is safer
        tag = space3.sub(
            "' >",
            tag,
        )
        ...
        for each in space4.findall(tag):
            tag = tag.replace(
                each,
                each[0] + " " + each[1:].lstrip(),
            )
        # remove quotes on some attributes
        tag = quotes_in_tag.sub(
            r"\1=\2 ",
            tag,
        )  # See Bug #28
