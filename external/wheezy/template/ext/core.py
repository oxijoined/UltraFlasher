...
def build_include(
    ...
    builder.add(
        lineno,
        "w(_r(" + value + ", ctx, local_defs, super_defs))",
    )
    return True
...
class CoreExtension(object):
    ...
    def __init__(
        ...
        self.lexer_rules: typing.Dict[
            ...
            200: (
                re.compile(r"%s(\w+(\.\w+)*)" % token_start),
                var_token,
            ),
            201: (
                re.compile(r"%s{(.*?)}" % token_start),
                rvalue_token,
            ),
            999: (
                re.compile(
