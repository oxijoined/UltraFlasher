...
class CleanSourceTestCase(unittest.TestCase):
    ...
    def test_clean_leading_whitespace(
        self,
    ) -> None:
    ...
    def test_leave_leading_whitespace(
        self,
    ) -> None:
...
class ParserTestCase(unittest.TestCase):
    ...
    def parse(
        ...
        return list(self.engine.parser.parse(self.engine.lexer.tokenize(source)))
...
class BuilderTestCase(unittest.TestCase):
    ...
    def build_source(
        ...
        nodes = list(self.engine.parser.parse(self.engine.lexer.tokenize(source)))
...
@end
"""
        assert "    Positive" == self.render(
            src,
            {"n": 1},
        )
        assert "    Zero" == self.render(
            src,
            {"n": 0},
        )
        assert "    Negative" == self.render(
            src,
            {"n": -1},
        )
...
@title()."""
    ...
    def test_def_syntax_error_compound(
        self,
    ) -> None:
        self.assertRaises(
            SyntaxError,
...
Thanks, John""" == self.render(
        ...
        assert "Thanks, John" == self.render(
            "footer.html",
            ctx,
        )
